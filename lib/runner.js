...
function findFiles(dir, ext, maxFiles = 150) {
  const skip = new Set(['node_modules', 'vendor', '.git', '__pycache__', 'dist', 'build', '.venv', 'venv', 'env', '.eggs', 'site-packages']);
  const result = [];
...
async function runLogic(repo, outDir) {
  ...
  // Go lint
  if (fs.existsSync(path.join(repo, 'go.mod')) && hasCommand('golangci-lint')) {
...
async function runEfficiency(repo, outDir) {
  ...
  if (hasCommand('radon')) {
    ...
    await runCmd('radon', ['hal', repo], { silent: true, ignoreError: true })
